<template>
    <section class="bg-gray-50 dark:bg-gray-900" v-if="open">
        ...
        <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" action="#">
                    ...
                    <button @click.prevent="login" type="submit" class="w-full text-white bg-primary-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Sign in</button>
...
<script setup>
  ...
  const login = () => {
    //"ali3@gmail.com", "password": "123456"
    if(password.value == '' || email.value == '') return;
    axios.post(config.public.API_URL + '/login/',{ "email": email.value, "password": password.value })
        .then((res) => {
            console.log(res)
